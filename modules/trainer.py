...
class BaseTrainer(object):
    ...
    @abstractmethod
    def _train_epoch(self, epoch, writer):
        raise NotImplementedError

    def train(self):
        writer = SummaryWriter(log_dir=os.path.join('./runs/', 'iu_xray'))

        not_improved_count = 0
        best_epoch = 0
        for epoch in range(self.start_epoch, self.epochs + 1):
            result = self._train_epoch(epoch, writer)
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch, writer):
        ...
        train_loss = 0
        self.model.train()
        # iu_xray:batch_size:16----130步,8----259步    7,470图片/2 = 3735
        # mimic_cxr: 8----33849步，16----16925步               473,057图片 没有两张一起
        ...
        writer.add_scalar('loss', train_loss / len(self.train_dataloader), epoch)

        self.logger.info('[{}/{}] Start to evaluate in the validation set.'.format(epoch, self.epochs))

        # # fixed the parameters in both the CNN backbone and the graph embedding module
        # for param in self.model.gcn.parameters():
